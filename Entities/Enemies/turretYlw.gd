...
func _draw():
	if current_state == STATES.ACTIVE:
		if turning == 0:
			draw_line(Vector2(-5,ray.position.y), (ray.get_collision_point() - position), Color8(255, 255, 0)/2, 1)
			if ray.is_colliding():
				var body = ray.get_collider()
				if body.is_in_group("player"):
					draw_line(Vector2(-5,ray.position.y), (ray.get_collision_point() - position), Color8(255, 255, 0), 1)
